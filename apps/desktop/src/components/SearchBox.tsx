...
const iconPaths: Record<IconName, string> = {
  search: 'M12 5.5a1 1 0 0 1 .71.29l4.5 4.5-1.42 1.42L13 9.42V18h-2V9.42l-2.79 2.29-1.42-1.42 4.5-4.5A1 1 0 0 1 12 5.5Z',
  compass: 'M12 4.5a7.5 7.5 0 1 0 7.5 7.5A7.51 7.51 0 0 0 12 4.5Zm3 4.2-1.21 3.67-3.58 1.47 1.21-3.67Zm-3 8.3a6 6 0 1 1 6-6 6 6 0 0 1-6 6Z',
  lightbulb: 'M12 3.5A5.5 5.5 0 0 0 8.71 13a2.75 2.75 0 0 0-.71 1.85v.4h8v-.4a2.75 2.75 0 0 0-.71-1.85A5.5 5.5 0 0 0 12 3.5Zm-2.5 13h5a1.5 1.5 0 0 1-1.35 1H10.85A1.5 1.5 0 0 1 9.5 16.5Zm.75 2h3.5a1.25 1.25 0 0 1-1.25 1h-1a1.25 1.25 0 0 1-1.25-1Z',
  globe: 'M12 4a8 8 0 1 0 8 8 8 8 0 0 0-8-8Zm5.73 7h-2.46a12.43 12.43 0 0 0-1-4.39A6.05 6.05 0 0 1 17.73 11ZM12 6c.62 0 1.73 1.72 2.25 4.5h-4.5C9.27 7.72 11.38 6 12 6ZM6.27 13h2.46a12.43 12.43 0 0 0 1 4.39A6.05 6.05 0 0 1 6.27 13Zm2.46-2H6.27a6.05 6.05 0 0 1 3.46-4.39A12.43 12.43 0 0 0 8.73 11Zm4.27 6c-.62 0-1.73-1.72-2.25-4.5h4.5C14.73 15.28 12.62 17 12 17Zm2.54-.11A12.43 12.43 0 0 0 14.27 13h2.46a6.05 6.05 0 0 1-3.46 4.39Z',
...
const engineConfig: Record<SearchEngine, { icon: IconName; label: string; description: string }> = {
  google: {
    icon: 'globe',
    label: 'Google Search',
    description: '',
...
const SearchBox = () => {
  ...
  const [status, setStatus] = useState<{ stage?: string; message?: string; elapsed?: number } | null>(null)
  const [clarPrompt, setClarPrompt] = useState<string | null>(null)
  const [clarText, setClarText] = useState<string>('')
  const [clarRewritten, setClarRewritten] = useState<{ original?: string; rewritten?: string; added?: string } | null>(null)
  ...
  useEffect(() => {
    const handler = (_: any, evt: any) => {
      ...
      if (sessionId && evt.sessionId === sessionId) {
        if (evt.type === 'status') {
          setStatus({ stage: evt.payload?.stage, message: evt.payload?.message, elapsed: evt.payload?.elapsed })
          ...
          setEvents(prev => [...prev, { type: 'cancelled', payload: evt.payload }])
          setStatus(null)
          setSessionId(null)
          setClarPrompt(null)
          setClarText('')
          ...
          const msg = typeof evt.payload === 'object' ? evt.payload?.message : String(evt.payload ?? '')
          const text = msg || 'Please provide additional details to clarify your request.'
          setClarPrompt(text)
          setEvents(prev => [...prev, { type: 'clarification_request', payload: text }])
        } else if (evt.type === 'clarification_rewrite') {
          const payload = evt.payload || {}
          const rewritten = payload.rewritten
          setClarRewritten({ original: payload.original, rewritten, added: payload.added })
          if (rewritten) {
            setEvents(prev => [...prev, { type: 'clarified_query', payload: rewritten }])
          }
        } else if (evt.type) {
          setEvents(prev => [...prev, { type: evt.type, payload: evt.payload }])
        }
      }
    }
  ...
  const handleCancel = async () => {
    ...
    try {
      ...
      setEvents(prev => [...prev, { type: 'cancel-error', payload: e instanceof Error ? e.message : 'Cancel failed' }])
    }
  }
  ...
  return (
    <section className="search-wrapper" aria-label="Deep Research prompt" style={{ paddingBottom: !idle ? 120 : 0 }}>
      <form className={`search-shell${!idle ? ' fixed-bottom' : ''}`} onSubmit={handleSubmit}>
          ...
          <div className="menu-selector engine-selector" ref={engineMenuRef}>
            ...
            {engineMenuOpen ? (
              <div className="menu-dropdown engine-menu" role="menu">
                {(Object.keys(engineConfig) as Array<SearchEngine>).map(option => (
                  <button
                    key={option}
                  ...
                  >
                    ...
                    <div className="menu-item-labels">
                      <span className="menu-item-title">{engineConfig[option].label}</span>
                      <small className="menu-item-desc">{engineConfig[option].description}</small>
                    </div>
                    <span className={`menu-toggle${engine === option ? ' on' : ''}`} aria-hidden="true" />
                  </button>
                ))}
              </div>
            ) : null}
                ...
                {modelOptions.map(option => (
                  ...
                  >
                    <div className="menu-item-labels">
                      ...
                      {option.description ? (
                        <small className="menu-item-desc">{option.description}</small>
                      ) : null}
                    </div>
                    <span className={`menu-toggle${model === option.id ? ' on' : ''}`} aria-hidden="true" />
                  </button>
                ))}
              </div>
            ) : null}
        ...
        {!idle && status?.message ? (
          <span className="inline-status" title={status.stage || ''} aria-live="polite">
            <span className="spinner" aria-hidden="true" />
            <span className="status-text">
              {status.message}
              {typeof status.elapsed === 'number' ? ` (${status.elapsed}s)` : ''}
            </span>
          </span>
        ) : null}
        {clarPrompt && sessionId ? (
          <button className="search-icon-btn" type="submit" aria-label="Submit clarification" disabled={!canSubmit}>
            <svg viewBox="0 0 24 24" aria-hidden="true">
              <path d={iconPaths.search} />
            </svg>
          </button>
        ) : isRunning ? (
          <button className="search-icon-btn stop" type="button" aria-label="Stop" onClick={handleCancel}>
            <svg viewBox="0 0 24 24" aria-hidden="true">
              {/* stop square */}
          ...
          <button className="search-icon-btn" type="submit" aria-label="Search" disabled={!canSubmit}>
            <svg viewBox="0 0 24 24" aria-hidden="true">
              <path d={iconPaths.search} />
            </svg>
          </button>
      ...
      <p className="search-helper" style={{ display: idle ? 'block' : 'none' }}>Discover facts, synthesize insights, and craft in-depth plans.</p>
      {!idle && events.length > 0 ? (
        <div className="search-output stream-output" role="status" aria-live="polite" ref={streamRef}>
          <span className="search-output-label">Reasoning Stream</span>
          <pre>{events.slice(-STREAM_MAX_EVENTS).map((e) => `${e.type}: ${typeof e.payload === 'string' ? e.payload : JSON.stringify(e.payload)}`).join('\n')}</pre>
        </div>
      ) : null}
