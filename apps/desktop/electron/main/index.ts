...
function resolvePythonExecutable(): string {
  ...
  const bin = (p: string) =>
    path.join(p, isWin ? "python.exe" : path.join("bin", "python"));
...
ipcMain.handle(
  ...
  async (
    ...
    // Map the UI-selected engine to the backend `search_engine` argument
    const mapEngineToBackend = (e?: string): string | undefined => {
      const val = (e || "auto").toLowerCase();
    ...
    const mapped = mapEngineToBackend(engine);
    const argsOverride: Record<string, any> = { ...(payload.args || {}) };
    if (mapped) argsOverride.search_engine = mapped;
    ...
    const finalResult = await new Promise<{ sessionId: string; result?: any }>(
      (resolve, reject) => {
        let buffer = "";
        let stderr = "";
        ...
        try {
          if (fs.existsSync(cancelFile)) fs.rmSync(cancelFile);
        } catch {}
        ...
        const child = spawn(
          ...
          [
            scriptPath,
            JSON.stringify({ ...pythonPayload, cancel_file: cancelFile }),
          ],
          {
        ...
        child.stdout?.on("data", (chunk) => {
          ...
          while ((idx = buffer.indexOf("\n")) !== -1) {
            ...
            try {
              ...
              evt.sender.send("reasoning-event", {
                sessionId,
                type: "log",
                payload: { line },
              });
...
ipcMain.handle(
  "send-clarification",
  async (evt, payload: { sessionId: string; text: string }) => {
    ...
    try {
      const msg =
        JSON.stringify({ type: "clarification_answer", payload: { text } }) +
        "\n";
...
ipcMain.handle(
  "cancel-session",
  async (evt, payload: { sessionId: string }) => {
    ...
    try {
      ...
      try {
        const cancelFile =
          (child.spawnargs?.find?.(() => false), process.env.DR_CANCEL_FILE); // backup if accessible
