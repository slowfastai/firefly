...
"""
This module is adapted from yt-dlp's `cookies.py`.
...
def load_cookies(browser_specification=("chrome",), cookie_file=None):
    """
    Load cookies from a file(cookie_file) or a browser(browser_specification).

    Args:
        cookie_file: The file to load cookies from.
        browser_specification: The browser to load cookies from.
                               A tuple containing the name of the browser, the profile
    ...
    try:
        ...
        if browser_specification is not None:
            ...
            cookie_jars.append(
                extract_cookies_from_browser(
                    browser_name, profile, keyring=keyring, container=container
                )
            )
        ...
        if cookie_file is not None:
            ...
            jar = DeepResearchCookieJar(cookie_file)
        ...
        # TODO: Should we return an empty cookie jar here instead of raising an Exception directly?
...
def extract_cookies_from_browser(
    ...
    Args:
        ...
        profile: The profile name/path from where cookies are loaded.
    ...
    if browser_name == "firefox":
        return _extract_firefox_cookies(profile, container)
    elif browser_name == "safari":
        return _extract_safari_cookies(profile)
    elif browser_name in CHROMIUM_BASED_BROWSERS:
        return _extract_chrome_cookies(browser_name, profile, keyring)
    else:
        logger.error(
            f"unknown browser: {browser_name} when extracting cookies from browser"
        )
        raise ValueError(f"unknown browser: {browser_name}")


def _extract_firefox_cookies(profile, container):
    """
    Internal function to extract cookies from a firefox browser.
    ...
    if cookie_database_path is None:
        logger.error(f"could not find firefox cookies database in {search_root}")
        raise FileNotFoundError(
            f"could not find firefox cookies database in {search_root}"
        )
    logger.info(f'Extracting cookies from newest: "{cookie_database_path}"')
    ...
    if container not in (None, "none"):
        ...
        if not os.path.isfile(containers_path) or not os.access(
            ...
            logger.error(f"could not read containers.json in {search_root}")
    ...
    with tempfile.TemporaryDirectory(prefix="yt_dlp") as tmpdir:
        ...
        try:
            ...
            if isinstance(container_id, int):
                ...
                logger.info("Only loading cookies not belonging to any container")
                cursor.execute(
...
def _extract_chrome_cookies(browser_name, profile, keyring):
    """
    Internal function to extract cookies from a chrome browser.
    ...
    with tempfile.TemporaryDirectory(prefix="yt_dlp") as tmpdir:
        ...
        try:
            ...
            logger.info(f"cookie version breakdown: {counts}")
            return jar
            ...
            if os.name == "nt" and error.errno == 13:
                ...
                raise Exception(message)  # force exit
            raise
        finally:
            if cursor is not None:
                cursor.connection.close()
...
def get_cookie_decryptor(
    browser_root, browser_keyring_name, *, keyring=None, meta_version=None
):
    ...
    return LinuxChromeCookieDecryptor(
        browser_keyring_name, keyring=keyring, meta_version=meta_version
    )
...
class LinuxChromeCookieDecryptor(ChromeCookieDecryptor):
    ...
    def _v11_key(self):
        password = _get_linux_keyring_password(
            self._browser_keyring_name, self._keyring
        )
    ...
    def decrypt(self, encrypted_value):
        ...
        if version == b"v10":
            ...
            logger.warning(f'unknown cookie version: "{version}"', only_once=True)
...
class MacChromeCookieDecryptor(ChromeCookieDecryptor):
    ...
    def decrypt(self, encrypted_value):
        ...
        if version == b"v10":
            ...
            return _decrypt_aes_cbc_multi(
                ciphertext, (self._v10_key,), hash_prefix=self._meta_version >= 24
            )
...
def _extract_safari_cookies(profile):
    """
    Internal function to extract cookies from a safari browser.
    ...
    if sys.platform != "darwin":
        logger.error(
            f"unsupported platform: {sys.platform} when extracting cookies from safari"
        )
        raise ValueError(
            f"unsupported platform: {sys.platform} when extracting cookies from safari"
        )
    ...
    if profile:
        ...
        if not os.path.isfile(cookies_path):
            logger.error(f"custom safari cookies database not found: {cookies_path}")
            raise FileNotFoundError(
                f"custom safari cookies database not found: {cookies_path}"
            )
        ...
        if not os.path.isfile(cookies_path):
            ...
            if not os.path.isfile(cookies_path):
                logger.error(f"could not find safari cookies database: {cookies_path}")
                raise FileNotFoundError(
                    f"could not find safari cookies database: {cookies_path}"
                )

    with open(cookies_path, "rb") as f:
        cookies_data = f.read()

    jar = parse_safari_cookies(cookies_data)
...
def _parse_safari_cookies_header(data):
    p = DataParser(data)
...
def _parse_safari_cookies_page(data, jar):
    p = DataParser(data)
    p.expect_bytes(b"\x00\x00\x01\x00", "page signature")
    number_of_cookies = p.read_uint()
    record_offsets = [p.read_uint() for _ in range(number_of_cookies)]
    if number_of_cookies == 0:
        logger.info(f"a cookies page of size {len(data)} has no cookies")
        return
    ...
    for i, record_offset in enumerate(record_offsets):
        ...
        record_length = _parse_safari_cookies_record(data[record_offset:], jar)
        p.read_bytes(record_length)
    p.skip_to_end("space in between pages")


def _parse_safari_cookies_record(data, jar):
    p = DataParser(data)
...
def parse_safari_cookies(data, jar=None):
    """
    References:
    ...
    if jar is None:
        jar = DeepResearchCookieJar()
    page_sizes, body_start = _parse_safari_cookies_header(data)
    p = DataParser(data[body_start:])
    for page_size in page_sizes:
        _parse_safari_cookies_page(p.read_bytes(page_size), jar)
...
def _choose_linux_keyring():
    ...
    logger.info(f"detected desktop environment: {desktop_environment.name}")
...
def _get_kwallet_network_wallet(keyring):
    ...
    try:
        ...
        if returncode:
            ...
            logger.info(f'NetworkWallet = "{stdout.strip()}"')
            return stdout.strip()
...
def _get_kwallet_password(browser_keyring_name, keyring):
    logger.info(f"using kwallet-query to obtain password from {keyring.name}")
...
def _get_gnome_keyring_password(browser_keyring_name):
    if not secretstorage:
        logger.error(f"secretstorage not available")
        return b""
...
def _get_linux_keyring_password(browser_keyring_name, keyring):
    ...
    logger.info(f"Chosen keyring: {keyring.name}")
...
def _get_mac_keyring_password(browser_keyring_name):
    logger.info("using find-generic-password to obtain password from OSX keychain")
    try:
        stdout, _, returncode = Popen.run(
            [
                "security",
...
def _get_windows_v10_key(browser_root):
    ...
    if path is None:
        ...
        return None
    logger.info(f'Found local state file at "{path}"')
...
def _decrypt_aes_cbc_multi(
    ciphertext, keys, initialization_vector=b" " * 16, hash_prefix=False
):
    for key in keys:
...
def _merge_cookie_jars(jars):
    output_jar = DeepResearchCookieJar()
    for jar in jars:
        for cookie in jar:
...
class DeepResearchCookieJar(http.cookiejar.MozillaCookieJar):
    """
    This is a modified version of the original YoutubeDLCookieJar.
